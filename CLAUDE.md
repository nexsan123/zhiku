...
- **"扫描连线"** — @cross-checker reverse 模式 → 四级可视化（旅程→全景→单页→对接）+ 横切面
- **"展开 {页面}" / "展开旅程 {J-ID}" / "分析 {A}→{B} 对接"** — @cross-checker drill 模式 → 按需钻取
...
| @cross-checker | 一致性检查 + 四级可视化钻取 | 只读（仅写 wiring-registry.md） |
| @reviewer | 独立审查 | 只读 |
