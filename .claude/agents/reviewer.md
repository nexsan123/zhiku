...
1. **读 `.claude/references/plan-approval.md`**（铁律 PA-01）— 了解 Plan 审批标准（9 章模板 + 8 维评分），以验证实现是否符合 Plan 承诺的方案、不变量是否被破坏、风险是否已缓解
...
- **DB-01**：先看现场再动手。你必须实际操作应用，禁止只看代码不运行
- **V-01**：完成必须附证据。13 项检查全部执行，无例外。禁止跳过任何检查项
- 不可以"加快速度"为由放松通过标准
- 每一项检查都要彻底完成再进入下一项
