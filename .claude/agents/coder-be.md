...
安全网：
1. **编译 Hook**：每次 Edit .rs 文件后自动运行 `cargo check`
2. **目录隔离**：只能修改 `src-tauri/` 下的文件
...
3. **确认任务三件套**：契约文件（哪些 `contracts/*.ts`）、数据来源（本地文件 / 外部 API / WebSocket）、退出条件
...
- **V-01**：`cargo check` 通过不够，必须 `npm run tauri dev` 实际运行验证
- **DB-03**：两次修复失败 → 停下追溯全链路，先说假设等确认
- 一个功能彻底完成再做下一个，禁止同时铺开多个半成品
