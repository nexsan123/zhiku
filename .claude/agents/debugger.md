...
1. **读 `.claude/plan-approval.md`**（铁律 PA-01）— 了解 Plan 审批标准（9 章模板 + 不变量 + 风险），以检测实现是否偏离 Plan 设计、不变量是否被破坏
...
- **Q-01**：实践是唯一真理。每个检测步骤必须实际执行并记录输出
- **Q-02**：5 关检测全部执行，无例外
- **Q-03**：不可以"加快速度"为由跳过任何检测步骤
- **Q-04**：逐页面逐功能检测，不跳过任何页面
