...
1. **读 `.claude/references/plan-approval.md`**（铁律 PA-01）— 进入 Plan Mode 写 Plan 前必读，包含强制 9 章模板、UI-1~UI-5 专属技术审视项、量化门槛、8 维评分标准。不符合模板的 Plan 会被退回
...
### ★ 可视化结构图（v7.0 铁律，2026-03-18 新增）

**所有设计提案必须包含可视化、透明化的结构图，这是用户确认"是否符合要求和目的"的唯一依据。**

必须产出的结构图：

1. **页面布局结构图**：用 ASCII / Markdown 表格清晰展示区域划分、比例、层级关系
   ```
   ┌─────────────────────────────────┐
   │  Header (64px)                  │
   ├────────┬────────────────────────┤
...
2. **组件关系图**：数据流向、状态依赖、父子关系
3. **配色/间距系统对照表**：token 名 → 实际值 → 使用场景，一目了然

**没有可视化结构图的设计提案 → Lead 直接退回，不进入用户确认环节。**
...
**退出条件**：用户对每一页的设计方案（含结构图）都说了"确认"或"可以"才能进入 X.1c 定稿。
...
- **DB-01**：先看现场再动手。设计提案必须经用户确认，不可自行拍板
- **V-01**：完成必须附证据。不可跳过任何阶段。设计审查（X.4）是必要步骤，不可省略
- 不可以"加快速度"为由省略三态设计或多语言考虑
- 一个页面的设计彻底完成再做下一个
